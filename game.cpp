...
Game::Game() :
	...
	map2(0.f, 0.f), game_over_screen(IMAGES_FOLDER + GAME_OVER_FILE_NAME, 1.634f, 0.98f),text1("res/kenvector_future.ttf", 24, 10, 5, sf::Color::Green),
	text2("res/kenvector_future.ttf", 24, 900, 5, sf::Color::Yellow)
{
	window.setVerticalSyncEnabled(true);
	for (size_t i = 0; i < CACTUS_QTY; i++) {
		Cactus* cactus = new Cactus( -WINDOW_WIDTH + rand() % (static_cast<int>(
			WINDOW_WIDTH - rand()%151+300))
...
void Game::update() {
	...
	case PLAY:
		map1.update();
		map2.update();
		text1.update(std::to_string(player.getHP()));
		text2.update(std::to_string(player.Distance()));
		player.update();
		for (size_t i = 0; i < CACTUS_QTY; i++) {
			cactuses[i]->update();
		}
