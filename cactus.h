...
public:
	...
	sf::FloatRect getHitBox() { 
		...
		position.x = sprite.getPosition().x  + (g_bounds.width - g_bounds.width/2)/2;
		position.y = sprite.getPosition().y  + (g_bounds.height - g_bounds.height / 2)/2;
		
		//формирование нового прямоугольника
		sf::Vector2f size(g_bounds.width / 2, g_bounds.height / 2);
		sf::FloatRect r1(position,size);
